...
export default function MobilisersClient() {
  const [users, setUsers] = useState<CM[]>([]);
  ...
  async function toggleActive(u: CM) {
    try {
  ...
  return (
    <div className="space-y-4">
        ...
        <Card>
          <form onSubmit={create} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {SETTLEMENTS.map((s) => (
                  <label key={s.code} className="flex items-center gap-2 rounded-lg border border-zinc-200 px-2 py-1.5 text-sm dark:border-zinc-800">
                    <input
                      type="checkbox"
                      checked={communities.includes(s.code)}
                      onChange={() => toggleCommunity(s.code)}
                    />
            ...
            <button type="submit" className={btnPrimary} disabled={saving}>
              {saving ? "Creating…" : "Create mobiliser"}
            </button>
          </form>
        </Card>
      )}
      ...
      {loading ? (
        ...
        <Empty>No mobilisers yet. Create the first CM login.</Empty>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold text-zinc-900 dark:text-zinc-50">
                    {u.name}{" "}
                    {u.mobiliserCode && (
                      <span className="text-xs text-zinc-400">· {u.mobiliserCode}</span>
                    )}
                  </p>
                  ...
                  <p className="mt-0.5 text-xs text-zinc-400">
                    ...
                    {u.communities.length
                      ? u.communities
                          .map(
                            (c) =>
                              SETTLEMENTS.find((s) => s.code === c)?.label || c
                          )
                          .join(", ")
                      : "no settlements"}
                  </p>
