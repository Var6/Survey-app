...
export default function MonthlyReportForm() {
  ...
  const caseStats =
    (report?.dashboard as { caseStats?: unknown } | null)?.caseStats as
  ...
  async function save(): Promise<boolean> {
    ...
    setSaving(true);
    try {
      await apiFetch(`/api/monthly/${report.id}`, {
        method: "PATCH",
  ...
  async function submit() {
    ...
    const ok = await save();
    if (!ok) return;
    const res = await fetch(`/api/monthly/${report.id}/submit`, { method: "POST" });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      {caseStats && (
        <div>
          <h2 className="mb-2 text-sm font-bold text-zinc-900 dark:text-zinc-50">
            Action cases &amp; service follow-up (auto)
          </h2>
          <CaseStatsTable rows={caseStats} />
        </div>
      )}
